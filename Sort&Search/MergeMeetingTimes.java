...
public class Solution {
    ...
    public static List<Meeting> mergeRanges(List<Meeting> meetings) {
        ...
        List<Meeting> op = new ArrayList<>();
        op.add(meetings.get(0));
        
        for(int i=0; i<meetings.size(); i++){
            
           Meeting current=meetings.get(i);
           Meeting recentOp=op.get(op.size()-1);
           
           if(recentOp.getEndTime()>=current.getStartTime())
            recentOp.setEndTime(Math.max(current.getEndTime(),recentOp.getEndTime()));
